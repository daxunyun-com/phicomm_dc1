...
smartconfig_done(sc_status status, void *pdata) {
	...
	case SC_STATUS_LINK_OVER:
		...
		if (pdata != NULL) {
            ...
            uint8 data[16]={0};
            os_sprintf(data,"%d.%d.%d.%d", phone_ip[0],phone_ip[1],phone_ip[2],phone_ip[3]);
            //system_param_save_with_protect(DATA_RECORD_START_PHONEIP, &data, sizeof(data));
            
            spi_flash_erase_sector(phone_ip_sec);
            if(spi_flash_write(phone_ip_sec * SPI_FLASH_SEC_SIZE, (uint32 *)&data, sizeof(data)) == SPI_FLASH_RESULT_OK){
                INFO("Phone ip successfully\r\n");
            }
            
            struct ip_info ipInfo;
            uint8 dev_ip[4] = {0};
            wifi_get_ip_info(STATION_IF, &ipInfo);
            dev_ip[0]=ipInfo.ip.addr;
            dev_ip[1]=ipInfo.ip.addr>>8;
            dev_ip[2]=ipInfo.ip.addr>>16;
            dev_ip[3]=ipInfo.ip.addr>>24;
            
            INFO("dev ip: %d.%d.%d.%d\n",dev_ip[0],dev_ip[1],dev_ip[2],dev_ip[3]);
            
            uint8 devData[16]={0};
            os_sprintf(devData,"%d.%d.%d.%d", dev_ip[0],dev_ip[1],dev_ip[2],dev_ip[3]);
            
            if(spi_flash_write(dev_ip_sec, (uint32 *)&devData, sizeof(devData)) == SPI_FLASH_RESULT_OK){
                INFO("dev ip successfully\r\n");
            }
		} else {
