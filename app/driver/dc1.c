...
void ICACHE_FLASH_ATTR reverse_switch(uint16_t switch_ret,uint8_t num){
    ...
    os_printf("switch_bit=10 switch_ret:%02x - %d\n",switch_ret,num);
    if(num == 0){
        ...
        os_printf("switch_bit else switch_ret:%0d - %02x - %02x\n",switch_bit,switch_ret,switch_ret & 0xF0 | 0x80);
...
u8 ICACHE_FLASH_ATTR set_switch(uint8_t num, bool bit_value){
    ...
    if(num == 0){
        ...
        if(bit_value == 1){
            switch_ret = switch_ret | switch_bit;
            os_printf("bit_value=1,switch_bit:%02x\n",switch_ret);
